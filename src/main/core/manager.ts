...
const coreLogNotificationRules: CoreLogNotificationRule[] = [
  {
    match: (event) => {
      ...
      return {
        ...
        title: t('main.notifications.tailscaleAuthRequired', { name }),
        body: t('main.notifications.tailscaleAuthOpenLink'),
        persistent: true,
        url,
        variant: 'warning'
      }
...
async function fallbackUnavailableServiceModes(reason: unknown): Promise<void> {
  ...
  try {
    ...
    void showNotification({ title: t('main.notifications.serviceUnavailableSwitched') })
  } finally {
    mainWindow?.webContents.reload()
