...
function getDelayText(delay: number, t: (key: string) => string): string {
  if (delay === -1) return t('detail.untested')
    ...
    >
      ...
      >
        ...
        <div className="px-3 py-2 grid grid-cols-[auto_1fr] gap-x-2 gap-y-1.5 items-center">
          <span className="text-[10px] text-muted">{t('detail.type')}</span>
          <Chip className="justify-self-end" variant="soft" size="sm">
            {proxy.type}
          </Chip>

          <span className="text-[10px] text-muted">{t('detail.status')}</span>
          <Chip
            ...
            size="sm"
          >
            {proxy.alive ? t('detail.online') : t('detail.offline')}
          </Chip>
          ...
          {proxy.udp !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.udp ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.tfo !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.tfo ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.xudp !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.xudp ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.mptcp !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.mptcp ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.smux !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.smux ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.uot !== undefined && (
            <>
              ...
              <Chip
                ...
                size="sm"
              >
                {proxy.uot ? t('detail.enabled') : t('detail.disabled')}
              </Chip>
            </>
          )}
          {proxy.interface && (
            <>
              <span className="text-[10px] text-muted">{t('detail.outboundInterface')}</span>
          ...
          {group?.now && !group?.fixed && (
            <>
              <span className="text-[10px] text-muted">{t('detail.currentSelection')}</span>
          ...
          {group?.fixed && (
            <>
              <span className="text-[10px] text-muted">{t('detail.fixedSelection')}</span>
              <span className="text-[10px] flag-emoji text-muted justify-self-end truncate">
                {group.fixed}
              </span>
            </>
        ...
        <div className="px-3 pt-2 pb-2.5">
          <span className="text-[10px] text-muted block mb-1.5">{t('detail.delayHistory')}</span>
          {history.length > 0 ? (
            ...
            <span className="text-[10px] text-muted/50">{t('detail.noHistory')}</span>
          )}
        </div>
      </Surface>
    </div>,
